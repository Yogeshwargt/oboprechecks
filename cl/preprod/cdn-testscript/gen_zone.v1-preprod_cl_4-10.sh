...
# Versions:
#   1.2 - Adapted to 4.8 and 4.9  
#--------------------------------------------------------------------
#
# Name: gen_zone v1.3
# Description: Script to create the zone file to be used with bind9
# Author: Leandro Magrassi - 16 Jul 2018
# Versions:
#   1.3 - Adapted to Preprod  
...
# --- OBO-CDN entries
declare -a cdn_list=("omwssu" "omw" "epg" "speedtest" "time")

# --- OBO-CDN new entries
declare -a cdn_list2=("staticqbr-$CC-$E.$SUBE.cdn")
declare -a cdn_list3=("obodrm.$E.pe")
...
echo "; ------ OBO-CDN entries >> domain=$EX"
for y in "${cdn_list2[@]}";
do
	nome=$(dig $y.$EX cname +short)
	#printf ">>>> $y.$EX"
	if [ -z $nome ]; then
		printf "%-23s %-15s %s\n" ";$y" "IN      CNAME" "****** UNABLE TO RESOLVE THIS ENTRY, PLEASE CHECK!!! ******"
	else
...
echo "; ------ CDN NEW entries 4.10 >> domain=.$SUBE.cdn.$EX"
